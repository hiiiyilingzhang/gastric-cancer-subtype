...
subset.m <- select(merged_gc, one_of(c("TCGA.subtype","NLRP3","AIM2","NLRC4","NLRP1","NLRP6","CASP1","GSDMD",
                                       "ASC","IL1B","IL17A","CD36","CXCL20","MPO","LY6G5C",
                                       "LY6G5B","PAD4","PSGL1")))
# Warning message:
#   Unknown columns: `ASC`, `CXCL20`, `PAD4`, `PSGL1`

# 2. Create Metadata for heapmap -----
rownames(subset.m) <- paste("Patient",1:199,sep = "-")

annotation_col <- data.frame(TCGA.Subtype = subset.m$TCGA.subtype)
rownames(annotation_col) <- paste("Patient",1:199,sep = "-")
head(annotation_col)
...
rownames(annotation_row) <- c("NLRP3","AIM2","NLRC4","NLRP1","NLRP6","CASP1","GSDMD","IL1B",
                              "IL17A","CD36","MPO","LY6G5C","LY6G5B")
head(annotation_row)

# 3. Heatmap -----
library(pheatmap)
subset.m <- arrange(subset.m, TCGA.subtype)
pdf(file= "results/TCGA-res.pdf")
